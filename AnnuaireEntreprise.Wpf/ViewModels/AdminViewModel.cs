...
public class AdminViewModel : INotifyPropertyChanged
{
    ...
    private async Task UpdateSiteAsync() { if (SelectedSite is null || string.IsNullOrWhiteSpace(SiteVille)){ MessageBox.Show("Sélectionnez un site et renseignez la ville."); return;} try { SelectedSite.Nom=SiteVille.Trim(); await _siteRepository.UpdateAsync(SelectedSite); await LoadSitesAsync(); MessageBox.Show("Site modifié avec succès."); } catch (Exception ex){ await _loggerService.LogErrorAsync($"Site update failed: {ex.Message}"); MessageBox.Show("Erreur lors de la modification du site."); } }
    private async Task DeleteSiteAsync() { if (SelectedSite is null){ MessageBox.Show("Sélectionnez un site."); return;} try { await _siteRepository.DeleteAsync(SelectedSite.Id); SiteVille=string.Empty; await LoadSitesAsync(); MessageBox.Show("Site supprimé avec succès."); } catch (Exception ex){ await _loggerService.LogErrorAsync($"Site delete failed: {ex.Message}"); MessageBox.Show("Erreur lors de la suppression du site."); } }
    ...
    private async Task UpdateServiceAsync() { if (SelectedService is null || string.IsNullOrWhiteSpace(ServiceNom)){ MessageBox.Show("Sélectionnez un service et renseignez le nom."); return;} try { SelectedService.Nom=ServiceNom.Trim(); await _serviceRepository.UpdateAsync(SelectedService); await LoadServicesAsync(); MessageBox.Show("Service modifié avec succès."); } catch (Exception ex){ await _loggerService.LogErrorAsync($"Service update failed: {ex.Message}"); MessageBox.Show("Erreur lors de la modification du service."); } }
    private async Task DeleteServiceAsync() { if (SelectedService is null){ MessageBox.Show("Sélectionnez un service."); return;} try { await _serviceRepository.DeleteAsync(SelectedService.Id); ServiceNom=string.Empty; await LoadServicesAsync(); MessageBox.Show("Service supprimé avec succès."); } catch (Exception ex){ await _loggerService.LogErrorAsync($"Service delete failed: {ex.Message}"); MessageBox.Show("Erreur lors de la suppression du service."); } }
